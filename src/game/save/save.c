...
static void fill_struct(win_t *win, save_elements_t *save_elements)
{
	...
	for (uint8_t i = 0; i < 4; i++) {
		save_elements->skills += skill[i]->level;
		save_elements->skills <<= 8;
	}
}


static uint8_t *read_bits(int32_t hex)
{
...
static void insert_struct(win_t *win, save_elements_t *save_elements)
{
	...
	for (int32_t i = 1; i < win->game->player->level; i++) {
		win->game->player->xp->max_value *= win->game->player->xp->mult;
		win->game->player->hp->max_value *= win->game->player->hp->mult;
	}
